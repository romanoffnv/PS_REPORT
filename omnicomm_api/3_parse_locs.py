...
def main():
    ...
    L_total_groups = cursor.execute(f"SELECT Groups FROM Groups_units").fetchall()
    L_total_units = cursor.execute(f"SELECT Units FROM Groups_units").fetchall()
    L_total_uuid = cursor.execute(f"SELECT id FROM Groups_units").fetchall()
    
    for i in online:
        if cursor.execute(f"SELECT id FROM Groups_units WHERE id like '%{i['uuid']}%'").fetchall():
    ...
    cursor.execute("DROP TABLE IF EXISTS Groups_units_status")
    df.to_sql(name='Groups_units_status', con=db, if_exists='replace', index=False)
    db.commit()
    db.close()
